...
*/
const DEFAULT_SERVER_URL = process.env.BETTER_AUTH_URL?.trim() || "https://dsh-guild-api.huiwang.fun";
/** 显式标注成 GuildSettings：register() 的 T 由 schema 推导，标注后与下游 scope 类型一致 */
const guildSettingsSchema = _deepseek_ai_schemastery.default.object({
...
*/
function seedServerUrl(ctx, scope) {
	if (envServerUrl()) return;
	const descriptor = ctx.settings.describe().find((entry) => entry.ns === GUILD_NS);
	if (descriptor?.user !== void 0 && "serverUrl" in descriptor.user) return;
	scope.update({ serverUrl: DEFAULT_SERVER_URL }).catch((error) => {
	...
	return [{
		...
		handler: async (_req, res) => {
			...
			try {
				...
				sendJson(res, 200, { sessions: (0, es_toolkit_array.orderBy)(snapshots, [(s) => s.header.createdAt], ["desc"]).slice(0, 200).map((snapshot) => {
...
function guildRoutes(ctx, scope) {
	return [
		{
			kind: "exact",
...
function apply(ctx) {
	const scope = ctx.settings.register(GUILD_NS, guildSettingsSchema, { applies: "live" });
	seedServerUrl(ctx, scope);
	const disposers = [];
	for (const route of guildRoutes(ctx, scope)) disposers.push(ctx.webServer.register(route));
	ctx.effect(() => () => {
